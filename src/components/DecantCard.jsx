...
export default function DecantCard({ perfume, onAbrir, onVender, onToggleAgotadoManual }) {
  ...
  const sinMl = (perfume.decant.mlDisponible || 0) <= 0;
  const agotadoManual = perfume.decant.agotadoManual === true;
  const agotado = sinMl || agotadoManual;
  const puedeMarcarAgotado = !sinMl && !!onToggleAgotadoManual;
  return (
    <div className="bg-white border border-neutral-200 rounded-xl p-4 flex gap-4">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between gap-2">
          ...
          {agotado && (
            <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-red-50 text-red-600 border border-red-200 shrink-0">
              {agotadoManual && !sinMl ? "Agotado (manual)" : "Agotado"}
            </span>
          )}
        </div>
        ...
        <div className="flex flex-wrap gap-1.5 mb-2">
          {tamanos.map((t) => (
            <button
              ...
              disabled={agotado || perfume.decant.mlDisponible < t}
        ...
        <div className="flex items-center gap-3">
          ...
          {puedeMarcarAgotado && (
            <button onClick={() => onToggleAgotadoManual(perfume)} className="text-[11px] text-neutral-500 hover:text-black underline underline-offset-2">
              {agotadoManual ? "Quitar marca de agotado" : "Marcar como agotado"}
            </button>
          )}
        </div>
      </div>
